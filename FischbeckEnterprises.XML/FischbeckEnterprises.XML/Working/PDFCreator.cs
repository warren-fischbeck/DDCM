...
namespace FischbeckEnterprises.XML.Working
{
    class PDFCreator
    {
        ...
        readonly string Main_Sheet = $"{System.IO.Directory.GetCurrentDirectory()}\\Source\\Main_Sheet.pdf";
        readonly string Details_Sheet = $"{System.IO.Directory.GetCurrentDirectory()}\\Source\\Details_Sheet.pdf";
        readonly string Spells_Sheet = $"{System.IO.Directory.GetCurrentDirectory()}\\Source\\Spells_Sheet.pdf";
        ...
        private async void BuildCharacterSheet()
        {
            using (PdfDocument _pdfDocument = new PdfDocument(new PdfReader(Main_Sheet), new PdfWriter(_FinishedCharacterSheet)))
            {
                ...
                form.GetField("hitdicetotal").SetValue($"").SetFontSize(FontSize(_character.HitDiceTotal.Length));
